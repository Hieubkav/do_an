...
@section('content')
    <div id="content" class="container mx-auto px-2 sm:px-4 ">
        <div class="bg-white border border-gray-300 rounded shadow-lg">
            ...
            <div class="p-4 sm:p-6 overflow-x-auto">
                ...
                <form action="{{ route('action_source') }}" method="POST">
                    @csrf
                    ...
                    <div class="relative overflow-x-auto">
                        <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                            ...
                            <tbody>
                                @php
                                    $count = ($sources->currentPage() - 1) * $sources->perPage();
                                @endphp
                                @foreach ($sources as $source)
                                    ...
                                    <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                                        ...
                                        <td class="px-6 py-4">{{ $source->products->count() }}</td>
                                        <td class="px-6 py-4 grid grid-cols-2 sm:grid-cols-1 gap-2">
                                            ...
                                            <a href="{{ route('delete_source', $source->id) }}"
                ...
                {{ $sources->links('pagination::tailwind') }}
            </div>
        </div>
    </div>
    ...
    <div id="myModal" class="fixed inset-0 items-center justify-center z-50 hidden">
        ...
        <div class="bg-white p-4 rounded-lg shadow-xl w-full max-w-md relative z-10">
            ...
            <div class="mt-3">
                <h2 class="text-xl font-semibold mb-3">Thêm xuất xứ</h2>

                <!-- Form thêm xuất xứ -->
                <form action="{{ route('store_source') }}" method="post">
                    @csrf
                    <div class="mb-4">
                        <label for="name" class="block text-sm font-medium text-gray-700 mb-2">Tên xuất xứ:</label>
    ...
    <div id="editModal" class="fixed inset-0 items-center justify-center z-50 hidden">
        ...
        <div class="bg-white p-4 rounded-lg shadow-xl w-full max-w-md relative z-10">
            ...
            <div class="mt-3">
                <h2 class="text-xl font-semibold mb-3">Sửa xuất xứ</h2>

                <!-- Form sửa xuất xứ -->
                <form action="{{ route('update_source', 'ID_HERE') }}" method="post" id="editForm">
                    @csrf
                    <div class="mb-4">
                        <label for="editName" class="block text-sm font-medium text-gray-700 mb-2">Tên xuất xứ:</label>
    ...
    <script>
        document.querySelectorAll('.openEditModalBtn').forEach(function(button) {
            button.addEventListener('click', function() {
                ...
                // Cập nhật action của form dựa trên ID của xuất xứ
