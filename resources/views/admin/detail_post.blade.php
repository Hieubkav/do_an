    ...
    <div id="content" class="container mx-auto p-6 min-h-screen">
        <div class="bg-white border border-gray-300 rounded shadow-lg p-6">
            ...
            <div class="flex flex-col md:flex-row justify-between items-center mb-6">
                <h5 class="text-3xl font-bold text-gray-700 mb-4 md:mb-0">Chỉnh sửa bài viết</h5>
            ...
            <!-- Form -->
            <form id="myForm" action="{{ route('update_post', $post->id) }}" method="POST" enctype="multipart/form-data">
                @csrf

                <!-- Title -->
                <div class="mb-4">
                    <label for="title" class="block text-gray-500 font-bold mb-2">Tên bài viết</label>
                ...
                <div class="mb-4">
                    <label for="pic" class="block text-gray-500 font-bold mb-2">Ảnh bài viết</label>
                    <input type="file" name="pic" id="pic"
                        class="w-full border rounded py-2 px-3 text-gray-700 focus:border-blue-500">
                    <div id="image-preview" class="mt-4">
                        <img src="{{ asset('asset/all_pic') }}/{{ $post->image->link }}" id="pic_show"
                ...
                <div class="mb-4">
                    ...
                    <textarea id="basic-conf" name="content"
                        class="w-full py-2 px-3 border border-gray-300 rounded focus:outline-none focus:border-blue-500">{{ $post->content }}</textarea>
                    @error('content')
                        <div class="mt-2 text-red-600">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="mb-4">
                    ...
                    <input
                        ...
                        type="text"
                        value="{{ $post->user->name }} - cập nhật lúc: {{ ('App\Helpers\Support')::change_date($post->updated_at) }}"
                        readonly>
                </div>
